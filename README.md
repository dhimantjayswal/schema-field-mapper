...
pipeline output, not hand-written. It currently scores 100% accuracy@1
against `data/gold_mapping.py` (`python evaluate_mapping.py`) — see
`WRITEUP.md` for what that number does and doesn't mean, and for a real
bug (a duplicate-destination collision on `dob`/`created_ts`) this build
caught and fixed. Re-run it yourself before submitting if you want a fresh
copy or want to try it against Claude.
